...
def plot_surface(decoder, path, epoch, shapename, resolution, mc_value, is_uniform_grid, verbose, save_html, save_ply,
...
def plot_cuts(points, decoder, path, epoch, near_zero, latent=None):
    ...
    position_cut = np.vstack(([xx, np.zeros(xx.shape[0]), yy]))
    position_cut = [position_cut + np.array([0., i, 0.]).reshape(-1, 1) for i in
                    np.linspace(min_y - 0.1, max_y + 0.1, 10)]
    for index, pos in enumerate(position_cut):
        # fig = tools.make_subplots(rows=1, cols=1)
